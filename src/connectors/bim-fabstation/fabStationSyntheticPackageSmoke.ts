...
export const runFabStationSyntheticPackageSmoke = (): NexusFabStationSyntheticPackageSmokeResult => {
  ...
  expect(intake.source.ifcProjectGlobalId === '0NXSFSPROJECT000000001', 'Synthetic IFCPROJECT GlobalId drifted.');
  ...
  expect(handOff.executionState === 'PACKET_PREPARED_NO_PARTNER_EXECUTION', 'Spatial hand-off must remain no-execution.');
  ...
  expect(packagePlan.executionState === 'PROJECT_PACKAGE_PLAN_READY_NO_UPLOAD', 'Package planner must remain no-upload.');
  ...
  expect(!simulatedComplete.assessment.boundaries.userVerifiedExternalEvidence, 'Synthetic evidence must not become real partner evidence.');
  ...
  expect(simulatedFailed.assessment.validationState === 'PARTNER_HANDOFF_REJECTED', 'FabStation failure must remain rejected.');
