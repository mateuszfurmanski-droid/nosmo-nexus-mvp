 ...
 *
 * Current auth stores OIDC `sub` as req.user.id. That value is accepted here only
 * as server-side provider input, immediately reduced to a SHA-256 lookup digest,
 * and never returned as a Nexus Person ID. Email/name matching is forbidden.
 */
    ...
    const rows = await db
      ...
      .where(
        and(
          ...
          eq(
            nexusIdentityBindingsTable.providerSubjectDigest,
