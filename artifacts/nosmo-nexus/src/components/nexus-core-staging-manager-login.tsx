...
export function NexusCoreStagingManagerLogin({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  const [status, setStatus] = useState("Paste your one-time staging claim to sign in.");
  ...
  const login = async () => {
    ...
    try {
      const response = await fetch(nexusCoreApiUrl("/api/nexus/core/staging-device-login"), {
        method: "POST",
