...
import { nexusCoreStagingHeaders, nexusCoreApiUrl, nexusCoreRequest } from "@/lib/nexus-core-staging-session";
...
export function NexusCoreApprovalPanel({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  const [inspected, setInspected] = useState<Record<string, any>>({});
  const [queue, setQueue] = useState<ApprovalRecord[]>([]);
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(nexusCoreApiUrl(`/api/nexus/core/projection?${params.toString()}`), {
        credentials: import.meta.env.VITE_NEXUS_CORE_API_ORIGIN ? "omit" : "include",
      ...
      const queueResult = await nexusCoreRequest(`approval-queue?${params}`);
      setQueue(queueResult.approvals); setInspected({});
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      window.removeEventListener("nexus:core-package-saved",onSession);
    };
  }, [refresh]);
  ...
  const decide = async (approvalId: string, decision: "approved" | "rejected") => {
    if (!inspected[approvalId]) { setStatus("Inspect the submitted evidence first."); return; }
    ...
    try {
      const response = await fetch(nexusCoreApiUrl(`/api/nexus/core/approvals/${encodeURIComponent(approvalId)}/decision`), {
        method: "POST",
        credentials: import.meta.env.VITE_NEXUS_CORE_API_ORIGIN ? "omit" : "include",
      ...
      setStatus("Decision not confirmed. Refresh the queue before retry.");
    } finally {
          ...
          {requestedApprovals.map((approval) => {
            ...
            return (
              <div key={approvalId} className="rounded-lg border border-amber-300/30 bg-amber-950/35 p-2">
                ...
                <button type="button" className="mt-2 text-xs underline" onClick={() => void nexusCoreRequest(`approvals/${encodeURIComponent(approvalId)}/evidence?${new URLSearchParams({projectId:PROJECT_ID,worldId:WORLD_ID})}`).then(value=>setInspected(current=>({...current,[approvalId]:value}))).catch(e=>setStatus(e.message))}>Inspect Evidence</button>
                {inspected[approvalId]?.evidence.map((e:any)=><p key={e.id} className="mt-1 text-xs">{e.title}: {e.answerText??e.linkedFileId??e.evidenceType}</p>)}
                <div className="mt-2 flex gap-2">
                  <button
                    type="button"
                    disabled={disabled || !inspected[approvalId]}
