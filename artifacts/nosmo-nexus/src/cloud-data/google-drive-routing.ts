...
function folder(role: NexusGoogleDriveFolderRole, id: string, title: string, canonicalUse: string): NexusGoogleDriveFolderRef {
  return { role, id, title, url: `https://drive.google.com/drive/folders/${id}`, canonicalUse };
}
...
function fromManifestFolder(
  ...
  return { role, id: source.id, title: source.name, url: source.url, canonicalUse: source.canonicalUse };
}

function normaliseProjectAlias(value: string) {
  return value.trim().toLowerCase().replace(/[_\s]+/g, "-").replace(/[^a-z0-9-]+/g, "").replace(/-+/g, "-").replace(/^-+|-+$/g, "");
}
...
export const NEXUS_GOOGLE_DRIVE_CLOUD_CONFIG = {
  ...
  globalFolders: {
    androidWorkModeInbox: folder("androidWorkModeInbox", "1yyVwt4WGHUlP81XZN_38RT9ic95A0YVh", "00_INBOX_FROM_ANDROID_WORK_MODE", "Initial Android Work Mode intake only. Files still require project resolution before canonical storage."),
    unclearPhotoReview: folder("unclearPhotoReview", "1teqLeR0bQJmSHyvkRmKwBODdoRrseJs5", "02_REVIEW_UNCLEAR_PHOTOS", "Photos or evidence requiring manual classification."),
    projectCandidateReview: folder("projectCandidateReview", "1OTapFre2kpxa4cELr_dES7V-IcJm5j4L", "03_OTHER_PROJECT_CANDIDATES", "Possible project material that has not reached a project boundary decision."),
    contactsCalendarReview: folder("contactsCalendarReview", "17sZPa2UPQSKgNaDPH28epfKFhPqqHCbd", "04_CONTACTS_CALENDAR_REVIEW", "Reviewed phone-derived context, not raw phone authority."),
    privateDoNotUpload: folder("privateDoNotUpload", "1aJEXvV92AJC06t1X3PBjWwmfn_ZjthxC", "05_PRIVATE_DO_NOT_UPLOAD", "Explicit privacy exclusion bucket. Never ingest into Project Graph."),
    projectWorldsRoot: fromManifestFolder("projectWorldsRoot", nexusCloudDriveManifest.roots.projectWorldsRoot),
    sharedRegistries: fromManifestFolder("sharedRegistries", nexusCloudDriveManifest.roots.sharedRegistries),
    connectorExports: fromManifestFolder("connectorExports", nexusCloudDriveManifest.roots.connectorExports),
    auditProvenance: folder("auditProvenance", "1XB3sxhtqtsGHYgXS4HhEs-BgLGaj56A9", "90_AUDIT_PROVENANCE", "Cloud-level audit and migration provenance."),
  },
  registries: {
    assetIndex: { role: "assetIndex", id: nexusCloudDriveManifest.registries.assetIndex.id, title: nexusCloudDriveManifest.registries.assetIndex.name, url: nexusCloudDriveManifest.registries.assetIndex.url },
    routingRules: { role: "routingRules", id: nexusCloudDriveManifest.registries.routingRules.id, title: nexusCloudDriveManifest.registries.routingRules.name, url: nexusCloudDriveManifest.registries.routingRules.url },
    migrationLog: { role: "migrationLog", id: nexusCloudDriveManifest.registries.migrationLog.id, title: nexusCloudDriveManifest.registries.migrationLog.name, url: nexusCloudDriveManifest.registries.migrationLog.url },
...
export function resolveNexusGoogleDriveProjectRoute(projectId: string, worldId?: string): NexusGoogleDriveProjectRoute | null {
  return projectRoutes.find((route) => route.projectId === projectId && (!worldId || route.worldId === worldId)) ?? null;
}
...
export function resolveNexusGoogleDriveProjectRouteFromAlias(projectIdOrAlias: string, worldId?: string): NexusGoogleDriveResolvedProjectRoute | null {
  ...
  if (directRoute) return { route: directRoute, requestedProjectId: projectIdOrAlias, requestedWorldId: worldId };
  ...
  return { route, requestedProjectId: projectIdOrAlias, requestedWorldId: worldId, matchedAlias: alias.alias };
}
...
export function isNexusGoogleDriveProjectBoundaryValid(projectId: string, worldId: string, folderId: string): boolean {
  ...
  return [route.projectRoot.id, route.inbox.id, route.pendingGraphLink.id, route.byTrade.id, route.byType.id, route.audit.id].includes(folderId);
}
