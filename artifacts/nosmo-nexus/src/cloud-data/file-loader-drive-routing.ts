...
export type NexusFileLoaderCloudRoute =
  ...
  | { mode: "provider-neutral"; projectId: string; worldId?: string };
...
export function resolveFileLoaderCloudRoute(projectId: string, worldId?: string): NexusFileLoaderCloudRoute {
  const project = findDriveManagedProject(projectId);
  if (!project) return { mode: "provider-neutral", projectId, worldId: worldId?.trim() || undefined };
