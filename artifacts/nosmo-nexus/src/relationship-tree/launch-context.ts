...
const APPROVED_SOURCES = new Set<RelationshipTreeLaunchContext["source"]>(["work-wallet"]);
...
export function parseRelationshipTreeLaunchContext(
  ...
  if (!APPROVED_SOURCES.has(source as RelationshipTreeLaunchContext["source"])) return null;
  if (!validNodeIds.has(focusNodeId)) return null;
  return { source: source as RelationshipTreeLaunchContext["source"], focusNodeId };
}
