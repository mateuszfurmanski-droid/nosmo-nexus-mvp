  ...
  return (
    <div className="space-y-5 pb-8">
        ...
        <div className="grid gap-6 p-5 md:grid-cols-[minmax(0,1fr)_320px] md:p-8">
          <div>
            <div className="flex flex-wrap items-center gap-2 text-xs font-semibold uppercase tracking-[.14em] text-primary">
              ...
              <span className="inline-flex items-center gap-1.5 text-muted-foreground"><MapPin className="h-3.5 w-3.5" /> Leeds, UK</span>
            </div>
            <h1 className="mt-5 text-3xl font-bold tracking-[-.04em] md:text-5xl">Riverside Heights</h1>
            <p className="mt-4 max-w-2xl text-sm leading-relaxed text-muted-foreground md:text-base">Residential refurbishment and compliance programme connecting doors, electrical commissioning, people, plans, tasks and evidence in one project workspace.</p>
            <div className="mt-6 flex flex-wrap gap-2">
              <span className="rounded-full border border-border bg-background/35 px-3 py-1.5 text-xs text-muted-foreground">Fire Doors & Joinery</span>
              <span className="rounded-full border border-border bg-background/35 px-3 py-1.5 text-xs text-muted-foreground">Electrical</span>
              <span className="rounded-full border border-border bg-background/35 px-3 py-1.5 text-xs text-muted-foreground">Compliance</span>
            </div>
          </div>
      ...
      {tab === "overview" && (
        <div className="grid gap-5 xl:grid-cols-[minmax(0,1.35fr)_minmax(330px,.65fr)]">
          <section className="rounded-3xl border border-border bg-card/55 p-5 md:p-6">
            <div className="flex items-end justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[.15em] text-primary">Project tools</p><h2 className="mt-1 text-xl font-semibold">Work inside Riverside Heights</h2></div><Link href="/trades" className="text-xs font-semibold text-primary">All trades</Link></div>
      ...
      {tab === "work" && (
        <section className="rounded-3xl border border-border bg-card/55 p-5 md:p-6">
          ...
          <div className="mt-5 overflow-hidden rounded-2xl border border-border">
            {openWork.map((item, index) => (
              <div key={item.title} className={`grid gap-3 bg-background/30 p-4 sm:grid-cols-[minmax(0,1fr)_150px_100px] sm:items-center ${index < openWork.length - 1 ? "border-b border-border" : ""}`}>
                <div><p className="text-sm font-semibold">{item.title}</p><p className="mt-1 text-xs text-muted-foreground">{item.owner} · {item.status}</p></div>
                <span className={`w-fit rounded-full border px-2.5 py-1 text-[10px] font-semibold ${item.priority === "High" ? "border-amber-400/25 bg-amber-400/10 text-amber-300" : "border-border text-muted-foreground"}`}>{item.priority} priority</span>
                <span className="inline-flex items-center gap-1.5 text-xs text-muted-foreground"><Clock3 className="h-3.5 w-3.5" /> {item.due}</span>
              </div>
            ))}
          </div>
        </section>
      ...
      {tab === "people" && (
        <section className="rounded-3xl border border-border bg-card/55 p-5 md:p-6">
          ...
          <div className="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            {people.map(person => (
              <Link key={person.name} href="/person-card-demo" className="rounded-2xl border border-border bg-background/35 p-5 hover:border-primary/35">
                <div className="flex h-12 w-12 items-center justify-center rounded-full border border-primary/30 bg-primary/10 text-sm font-bold text-primary">{person.initials}</div><h3 className="mt-4 font-semibold">{person.name}</h3><p className="mt-1 text-sm text-muted-foreground">{person.role}</p><p className="mt-4 text-xs font-semibold text-emerald-300">{person.status}</p>
              </Link>
            ))}
          </div>
        </section>
        ...
        <section className="rounded-3xl border border-border bg-card/55 p-5 md:p-6">
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2">
            {documents.map(document => (
              <div key={document.name} className="flex items-center gap-4 rounded-2xl border border-border bg-background/35 p-4"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-secondary text-primary"><FileText className="h-5 w-5" /></div><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold">{document.name}</p><p className="mt-1 text-xs text-muted-foreground">{document.type} · {document.updated}</p></div></div>
            ))}
          </div>
        </section>
      )}
      ...
      <section className="flex flex-col gap-4 rounded-3xl border border-primary/20 bg-primary/[.05] p-5 sm:flex-row sm:items-center sm:justify-between md:p-6">
        <div className="flex items-start gap-4"><div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-primary/10 text-primary"><Network className="h-5 w-5" /></div><div><p className="font-semibold">Every action remains attached to this project</p><p className="mt-1 text-sm text-muted-foreground">People, documents, tasks and specialist tools share the same Riverside Heights context.</p></div></div>
