          ...
          <div className="relative pt-8">
            ...
            <div className="space-y-4 mt-8 pt-6 border-t border-border">
              <div className="flex items-center gap-3 text-sm">
                ...
                <span className="text-foreground break-all">{person.email}</span>
              </div>
              <div className="flex items-center gap-3 text-sm">
                <div className="w-8 h-8 rounded-full bg-secondary flex items-center justify-center shrink-0">
            ...
            <div className="space-y-3">
              {tasks.slice(0, 3).map((task) => (
                <FocusableEntity
                  ...
                  className="flex items-start gap-2 text-sm group rounded-md -mx-1 px-1 py-1 hover:bg-secondary/50 transition-colors"
                >
                  ...
                  <div>
                    <p className="text-foreground group-hover:text-primary transition-colors">{task.title}</p>
        ...
        <div className="bg-card border border-border rounded-xl overflow-hidden">
          ...
          <div className="divide-y divide-border">
            {projects.length > 0 ? projects.map((proj) => (
              <FocusableEntity
                key={proj.id}
                target={{ type: "project", id: proj.id }}
                ariaLabel={`Open project ${proj.name}`}
                className="p-4 hover:bg-secondary/50 transition-colors flex items-center justify-between group"
              >
                <div>
            ...
            <div className="divide-y divide-border flex-1 overflow-y-auto max-h-80">
              {documents.length > 0 ? documents.map((doc) => (
                <FocusableEntity
                  ...
                  ariaLabel={`Open document ${doc.title}`}
                  className="p-4 hover:bg-secondary/50 transition-colors flex items-center gap-3 group"
                >
