 ...
 *
 * Authority remains server-side. This client never supplies actor Person, participation,
 * grants or access decisions; it sends only the recipient's existing Nexus Bearer session.
 */
final class NexusCoreWorkClient {
    ...
    static void startTask(Context context, String origin, String taskId, Callback callback) {
        ...
        request(context, origin, "POST", "/api/nexus/core/tasks/" + encodePath(taskId) + "/start", body, callback);
    }
    ...
    static void finishTask(
            ...
            String origin,
            String taskId,
            JSONArray completedChecklistItemIds,
            Callback callback
    ) {
        JSONObject body = baseMutationBody();
        try {
    ...
    private static JSONObject baseMutationBody() {
        ...
        try {
            ...
            // org.json only throws for invalid numeric values; all values above are bounded strings.
        }
        return body;
    }
        ...
        new Thread(() -> {
            ...
            try {
                ...
                String message = success
                        ? "Canonical Nexus operation confirmed"
                        : payload != null
                            ? payload.optString("message", payload.optString("error", "Nexus rejected the operation"))
                            : "Nexus rejected the operation (HTTP " + status + ")";
