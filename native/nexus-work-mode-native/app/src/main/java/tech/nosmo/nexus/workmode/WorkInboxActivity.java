...
/** Recipient projection for canonical Nexus Work Packages. No assignment state is owned locally. */
public final class WorkInboxActivity extends Activity {
    ...
    private void refreshInbox() {
        ...
        NexusCoreWorkClient.loadInbox(this, configuredNexusOrigin(), (success, httpStatus, message, payload) -> runOnUiThread(() -> {
    ...
    private LinearLayout renderTask(JSONObject task) {
        ...
        if (packageItems != null) {
            for (int index = 0; index < packageItems.length(); index++) {
                ...
                card.addView(text("• " + kind.toUpperCase() + " — " + item.optString("label", "Work item"), 12, TEXT, false), fullWidthWrap());
    ...
    private void setBusy(boolean busy, String message) { refresh.setEnabled(!busy); status.setText(message); }
    ...
    private LinearLayout.LayoutParams fullWidthWrap() { return new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT); }
