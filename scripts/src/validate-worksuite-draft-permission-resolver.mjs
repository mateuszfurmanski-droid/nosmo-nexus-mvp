...
assert(resolver.includes("actorContextAuthority: \"server-side-required\""), "resolver must require server-side actor authority");
assert(resolver.includes("clientActorContextTrusted: false"), "resolver must not trust client actor context");
assert(resolver.includes("pending-server-side-person-project-participation-lookup"), "server-side Person/Project Participation lookup placeholder missing");
assert(resolver.includes("Client-supplied actorContext is not trusted"), "client actor context warning missing");
assert(resolver.includes("x-nexus-demo-actor-fixture"), "explicit demo fixture header missing");
assert(resolver.includes("NEXUS_WORKSUITE_PERMISSION_DEMO_FIXTURES"), "demo fixture env gate missing");
assert(resolver.includes("explicit-demo-fixture"), "explicit demo fixture source missing");
assert(resolver.includes("Do not use this as production authority"), "demo fixture warning missing");
