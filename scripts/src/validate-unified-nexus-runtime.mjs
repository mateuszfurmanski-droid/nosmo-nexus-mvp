...
const cloudAuthority = read("artifacts/api-server/src/lib/nexus-cloud-access-authority.ts");
const runtimeIdentity = read("artifacts/api-server/src/lib/nexus-runtime-identity.ts");
...
assert(
  ...
  "deployment must not keep the unauthenticated scripts server as public runtime",
);
...
assert(
  jsonParser >= 0 && gatewayMount < jsonParser,
  "Work Wallet gateway must own its body before generic JSON parsing",
);
...
assert(app.includes('authRuntime: "api-server"'), "health must identify authenticated API runtime");
...
assert(!bridge.includes("allowedEventTypes"), "bridge must not copy Work Wallet business rules");
assert(
...
assert(runtimePaths.includes("process.env.NEXUS_RUNTIME_ROOT"), "runtime root must support explicit override");
assert(runtimePaths.includes("existsSync"), "runtime root candidates must be marker-verified");
assert(
  legacyGateway.includes("export async function handleWorkWalletApi"),
  "canonical existing Work Wallet gateway export is missing",
);

assert(
  runtimeIdentity.includes("resolveNexusRuntimeIdentityBinding"),
  "unified auth runtime must resolve exact canonical Person binding",
);
assert(
  !runtimeIdentity.includes("personId: req.user.id"),
  "provider subject must never be promoted directly to canonical personId",
);
assert(
  cloudAuthority.includes("resolveNexusServerRuntimeIdentity"),
  "Cloud authority must consume the unified server identity runtime",
);
assert(
  cloudAuthority.includes("loadNexusProjectAccessRows"),
  "Cloud authority must load server-owned Project access records",
);
assert(
  cloudAuthority.includes("resolveNexusCloudWriteAccess"),
  "Cloud authority must delegate to canonical Cloud access policy",
);
